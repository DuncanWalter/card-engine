...
export const engine = new Module('engine', ({ global, next }) => {
    ...
    registerReward('Heal 5 health points.', 1, function* heal(self, state): * {
        self.collected = true // TODO: need to dispatch this to state
    ...
    registerReward('Draft a Card', 2, function* draft(self, state): * {
        self.active = true // TODO: need to dispatch this to state
        navigateTo('/game/cardDraft')
    }, (self, level, seed) => {
        self.cards = CardLibrary.sample(Math.floor(3) + level / 5.5).map(C => new C())
        return self
    })
