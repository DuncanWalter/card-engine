...
import { Button, Block, Frame, Modal, Col, Row, Shim, Material } from "../utility"
...
const Display = Material.extend`
    ...
    cursor: ${ props =>
        props.available !== false ? 'pointer' : 'inherit' 
    };
`
...
const previewCharacter = registerOverlay(({ resolve, character }) => {
    return <Col shim>
        ...
        />
        <Row>
            <Button danger onClick={ click => resolve(undefined) }>Back</Button>
            <Button primary onClick={ click => resolve(character) }>Confirm</Button>
        </Row>
    </Col>
})
...
const selectCharacter = registerOverlay(({ resolve, character, index }) => {
    return <Col shim style={{ alignItems: 'center' }}>
        <h1>Select Character</h1>
        <Row style={{ flexWrap: 'wrap', maxWidth: '800px' }}>
            {[...characters.values()].filter(set => 
                set.playable && (!character.includes(set.name) || character[index] == set.name)
            ).map(character =>
                <Display 
                    available={ true }
    ...
    return <Modal>
        <OverlayContext match={ match }>
            <Col shim>
                ...
                <Shim/>
                <Row>
                    <Button danger  onClick={click => { history.goBack() }}>Back</Button>
                    <Button primary onClick={click => {
                        // TODO: Delete the blank
                        const blank = new Entity({}, toExtractor({}))
                        resolver.processEvent(StartGame(blank, blank, {
                            seed: 100345,
