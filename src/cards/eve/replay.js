...
export const Replay = defineCard('replay', playReplay, {
    energy: 1,
    block: 4,
    inHand: false,
}, {
    title: 'Replay',
    text: 'Gain #{block} block. On next play card, place card on top of draw pile.',
    
    color: '#dd7712',
})

// TODO: add upgraded logic
function* playReplay(self: Card<ReplayData>, { energy, game, actors, resolver }: PlayArgs){
    ...
    const bind = new BindEffect(actors, game.player, {
        Effect: ReplayEffectDP,
        stacks: 1,
    }, ReplayEffectDP, targeted)
...
const ReplayEffectH = defineEffect('replayHand', {
    description: 'The next #{stacks} cards played will be returned to your hand.',
    name: 'Replay',
    innerColor: '#dd7712',
    outerColor: '#dd7712',
    sides: 13,
    ...
    actors: [owner],
    tags: [PlayCard]
}), (owner, type) => function*({ data, resolver, cancel, actors, subject }){
...
}, [ExhaustCard], [AddToDiscardPile])

const ReplayEffectDP = defineEffect('replayDrawpile', {
    description: 'The next #{stacks} cards played will be placed on top of the draw pile.',
    name: 'Replay',
    innerColor: '#dd7712',
    outerColor: '#dd7712',
    sides: 13,
...
}, [ExhaustCard, ReplayEffectH], [AddToDiscardPile])
