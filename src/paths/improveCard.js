...
const improveCard = registerOverlay(({ resolve, game }) => {
    return <Col shim>
        ...
        <CardPanel
            cards={[...game.deck].filter(card => card.upgrades(game.player.sets).length)} 
            sets={game.player.sets} 
            onClick={ card => {
                selectUpgrade({ resolve, game, card }).then(upgrade => {
                    if(upgrade){
                        resolve(new ImproveCard(game.player, card, { 
                            from: game.deck,
                            upgrade,
                        }))
                    }
...
registerReward('Improve', 'Improve a Card', 3, function* improve(self, resolver, game){
