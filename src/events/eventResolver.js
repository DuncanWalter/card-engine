...
export class EventResolver {
  ...
  registerListenerType(
    ...
    if (this.initialized) {
      throw new Error(
        `Cannot register type ${type.toString()} with event resolver after the resolver is initialized.`
      )
      ...
      this.listenerOrder.set(type, {
        // $FlowFixMe
        parents: parents
          ? parents.map((p) => (typeof p == 'string' ? p : p.type))
          : [],
        // $FlowFixMe
        children: children
          ? children.map((c) => (typeof c == 'string' ? c : c.type))
          : [],
        index: -1,
        id: type,
        compare(e) {
  ...
  initialize(getGame: () => Game) {
    if (this.initialized) {
      throw new Error(
        'Event Resolver initialized twice. Event Resolvers may only be initialized once.'
      )
    }
    const order = topologicalSort([...this.listenerOrder.values()])
    order.forEach((e, i) => {
      e.index = i
    })
...
function applyInternals(ls: LL<Listener<any>>): LL<Listener<any>> {
  ...
  while ((listener = alv.list[0])) {
    if (listener && listener.internal) {
      ...
      listeners = listeners
        .filter((listener) => listener != wrapper)
        .map((listener: Listener<any>) => {
...
const processEvent = synchronize(function* processEvent(
  self: EventResolver,
  event: Event<any>
): Generator<Promise<any>, Event<any>, any> {
  ...
  const executionQueue: Listener<any>[] = activeListeners
    .toArray()
    .sort((a, b) => {
      // TODO: perform checks
  ...
  const next: () => Promise<void> = synchronize(function*(): Generator<
    any,
    any,
    any
  > {
...
const processQueue = synchronize(function* processQueue(
  self: EventResolver
